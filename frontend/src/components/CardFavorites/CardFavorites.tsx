...
const CardFavorites = () => {
    ...
    if (favoritos.length === 0) {
        return (
            <article className={`${styles.noFavorites} flex flex-col items-center gap-3 w-full`}>
                 <span>
                    <i
    ...
    return (
        <article className={`grid grid-cols-1 gap-3 p-2 ${styles.containerCardFavorites} w-4/6`}>
            {favoritos.map((personagem) => (
                <div 
                    key={personagem.id} 
                    className={`flex items-center gap-3 p-3 rounded-lg bg-[#1e1e1e] hover:bg-[#2a2a2a] transition-colors cursor-pointer ${styles.favoritesCharacter}`}
                    onClick={() => window.location.href = `/personagem/${personagem.id}`}
                >
                    <img 
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className={`text-gray-400 ${styles.cardBio}`}>
                            {personagem.bio}
                        </span>
